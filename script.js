...
//$(selectorName) Think of this part as if the .html file is a map. When we tell jQuery--> $(selectorName), it means, travel to the part of the html that matches this selector. Remember, selectors can be html tags, or ids, or classes! In this case, jQuery makes a trip to $('body'), the <body> element, each time our loop runs! Once jQuery is at any element, it can do different things to that element. For example, if you say $('body').hide(), jQuery will basically make your whole webpage disappear! 
...
//In our case, $('body').append ADDS html to our page. So, knowing what you know now, see if you can figure out how the argument to our .append() method works!

//Note: You'll often hear function and method used interchangeably. There are some (important) differences, but for now, think of functions as standalone calls like add(3,4), and methods as functions that have another variable/value plus a dot (.) in front of it like: $('body').append(...) or [1,2,3].push(4). All methods are functions, but not all functions are methods!
